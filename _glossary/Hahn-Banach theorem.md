...
Let $\mathcal{X}$ be an $\mathbb{R}$-[[vector space]]. Suppose $q : \mathcal{X} \to \mathbb{R}$ is a _quasi-seminorm_. Suppose also we are given a [[linear subspace|linear subspace]] $\mathcal{Y} \subset \mathcal{X}$ and a _linear map_ $\phi : \mathcal{Y} \to \mathbb{R}$, such that
...
## Theorem (_Helly, Hahn-Banach analytic form_)

Let $E$ be a [[vector space]] over $\mathbb{R}$.

A [[functional]] is a _function_ defined on $E$ or some _subspace_ of $E$ with values in $\mathbb{R}$.
...
Then under these assumptions, there exists a _linear functional_ $f$ defined on all of $E$ that _extends_ $g$, i.e. $g(x) = f(x) \forall x \in G$, and such that

- $f(x) \leq p(x) \quad \forall x \in E$

### Notes

- extension of _linear functional_ defined on a _linear subspace_ of $E$ by a _linear functional_ defined on all of $E$
