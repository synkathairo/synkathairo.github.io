...
(i.e. the *concept class*/*hypothesis class* $\mathcal{F}$ can realize all possible subsets of $T$, or there is a hypothesis $S$ that predicts every possible assignment)

$(U, \mathcal{F})$ has **VC** $\dim \leq d$  if no set of size $d+1$ can be *shattered*.
