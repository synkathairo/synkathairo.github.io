...
In other words, $\mathbf{P}$ is the set of [[decision problem|languages]] $L$ such that there exists a _polynomial-time_ algorithm $A$ with $x \in L \iff A(x) = 1$.

## Notes

- a notion of "efficient computation", equated with _polynomial_ running time
- It is known that $\mathbf{P} \subseteq \mathbf{NP}$ and $\mathbf{P} \subseteq \mathbf{coNP}$ (see [[class NP|NP]] and [[class coNP|coNP]])

## See also

- _Cobham-Edmonds thesis_
...
1. S. Arora, B. Barak. _Computational Complexity: A Modern Approach_, Cambridge University Press, 2009, p. 25.
