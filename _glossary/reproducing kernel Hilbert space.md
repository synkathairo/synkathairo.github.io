...
A **reproducing kernel Hilbert space** (_RKHS_) is a _Hilbert space_ $H$ with a _reproducing kernel_ whose [[span]] is [[dense]] in $H$.

Or equivalently, **RKHS** is a _Hilbert space_ of [[function|functions]] with all [[evaluation functional|evaluation functionals]] bounded and [[linear functional|linear]].
...
- $k$ has the _reproducing property_ (kernel trick)
...
- the _reproducing kernel_ $k : \mathcal{X} \times \mathcal{X} \to \mathbb{R}$ may be a [[kernel]] if it is symmetric and _positive semidefinite_ (i.e. its Gram matrix $K_{ij} = k(x_i,x_j)$ is [[positive semidefinite]])
