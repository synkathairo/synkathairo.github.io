...
- accessing any element of an array if we have its index (*address*) stored in a known location, this is the *random access memory* (RAM) component of the model. If array lengths $\geq m$, then location index requires $\lceil\log_2 (m) \rceil$ bits to write down, word size at least this large.
