...
An equivalent way to define _Bayesian equilibrium_ is via an auxiliary game, the [[agent-form game]].

## Theorem (Bayesian equilibrium and Nash equilibrium in corresponding agent-form game)

A [[strategy vector]] $\sigma^* = (\sigma_i^*)_{i \in N}$ is a _Bayesian equilibrium_ in a game $\Gamma$ [[Harsanyi game with incomplete information|with incomplete information]] if and only if the strategy vector $(\sigma_i^*(t_i))_{i \in N, t_i \in T_i}$ is a [[Nash equilibrium]] in the corresponding [[agent-form game]] $\hat{\Gamma}$.
