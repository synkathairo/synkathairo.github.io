...
Suppose $V$ is a [[vector space]], $L$ is a non-empty _subset_ of $V$. If there exists $v \in V$ such that $L + v = \{v+l | l \in L\}$ is a _vector subspace_ of $V$, then $L$ is a **linear manifold** of $V$

Say the _dimension_ of $L$ i s the dimension of $L+v$ i.e. $\dim L = \dim(L+v)$

## Notes

- in the case $\dim L = \dim V-1$, $L$ is called a [[hyperplane]]
- _linear manifold_ in other words is a [[linear subspace]] possibly shifted away from the origin
