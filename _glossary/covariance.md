...
for a *sample* of data on *random variables* we may define the **sample covariance matrix** as a $K$-by-$K$ matrix $\mathbf{Q} = [q_{jk}]$ where $$q_{jk} = \frac{1}{N-1}\sum_{i=1}^N (x_{ij}-\bar{x}_j)(x_{ik}-\bar{x}_k)$$ with $q_{jk}$ as estimate of *covariance* between $j$-th and $k$-th variables underlying the data; we may also write $$\mathbf{Q} = \frac{1}{N-1}\sum_{i=1}^N (\mathbf{x}_i-\bar{\mathbf{x}})(\mathbf{x}_i-\bar{\mathbf{x}})^T$$
...
- the *sample covariance* is an *unbiased estimate* of the *covariance matrix*
	- see also: *sample mean* (c.f. [[expectation|mean]])
