...
Alternatively,
Suppose $A \subset \mathbb{R}^N$ is nonempty, [[compact set|compact]], [[convex set|convex]] set. $f: A \to A$ [[upper hemicontinuity|u.h.c.]] with the property that $f(x) \subset A$ is nonempty and convex for every $x \in A$. Then $f(\cdot)$ has a *fixed point*: $\exists$ an $x \in A$ such that $x \in f(x)$.
