...
- set of [[pure strategy|pure strategies]] of player $t_i$ in $\hat{\Gamma}$ is $A_i(t_i)$, set of available *actions* of that [[type set|type]] in the game $\hat{\Gamma}$.
- [[utility function|payoff function]] $\hat{u}_{t_i}$ of player $t_i$ in $\hat{\Gamma}$ is given by $$\hat{u}_{t_i}(a) := \sum_{t_{-i} \in T_{-i}} p(t_{-i} \vert t_i) u_i(t_i, t_{-i}; (a_j(t_j))_{j \in N})$$ where $a = (a_j(t_j))_{j \in N, t_j \in T_j}$ denotes vector of actions of all players in $\hat{\Gamma}$.
