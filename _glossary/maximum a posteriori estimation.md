...
Let $x = (x_1,…,x_n)$ be i.i.d. realizations from [[probability mass function]] $p_X(t; \Theta = \theta)$ (if $X$ discrete), or from density $f_X(t; \Theta = \theta)$ (if $X$ continuous), where $\Theta$ is the [[random variable]] representing the parameter (or vector of parameters). We define the **maximum a posteriori** (MAP) estimator $\hat \theta_{MAP}$ of $\Theta$ to be the parameter which maximizes the _posterior_ distribution of $\Theta$ given the data.
