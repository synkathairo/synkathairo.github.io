...
Define the **Shapley value** as the [[solution concept]] $\operatorname{Sh}$, for [[coalitional game]] $(N; v)$ where $N$ is the set of players and $v(N)$ is the *worth* of the *grand coalition* $N$, $$\operatorname{Sh}_i(N;v) := \frac{1}{n!}\sum_{\pi \in \Pi(N)} (v(P_i(\pi) \cup \{i\}) - v(P_i(\pi))), \quad \forall i \in N$$
