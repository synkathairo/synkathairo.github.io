...
Consider a *valuation function*, which is a set function $f: 2^M \to \mathbb{R}$ over a set $M$ of $m$ items assigning a real value $f(S)$ to each set $S \subseteq M$, which is additionally [[monotone function|monotone]] ($f(T) \leq f(S)$ for every $T \subseteq S$) and *normalized* ($f(\emptyset) = 0$). Let $\mathbf{p} = (p_1,...,p_m) \in \mathbb{R}^m$ be a *price vector*. Then, the *demand* of $f$ under $\mathbf{p}$ is $D(\mathbf{p}) = \arg\max_{S \subseteq M} \{f(S) - \sum_{j \in S} p_j\}$.

A *valuation function* $f$ is **gross substitutes** (GS) if for every pair of *price vectors* $\mathbf{p},\mathbf{q}$ such that $\mathbf{p} \leq \mathbf{q}$, for every set $S \in D(\mathbf{p})$, there exists $T \in D(\mathbf{q})$ such that $T$ contains every item $j \in S$ such that $p_j = q_j$.
