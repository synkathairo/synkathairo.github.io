...
- $\mathcal{D}_{\mathsf{unif}} := \{(\mathbf{a}_i,b_i): \mathbf{a}_i \gets \mathbb{F}^k, b_i \gets \mathbb{F} \}_{i \in [n]}$
  for [[finite field]] $\mathbb{F}$, $\mathsf{Bin}_p$ _error distribution_ over $\mathbb{F}$ outputting random non-zero element of $\mathbb{F}$ with probability $p$ and $0$ with probability $1-p$.
