...
If $x^*$ is an _evolutionarily stable strategy_ in a two-player symmetric game, then $(x^*, x^*)$ is a symmetric [[Nash equilibrium]] in the game.

## Theorem (condition for ESS)

A strategy $x^*$ is _evolutionarily stable_ if and only if for each $x \neq x^*$ only one of the following two conditions obtains:
...
In a symmetric game, if $(x^*, x^*)$ is a strict symmetric equilibrium then $x^*$ is an _evolutionarily stable equilibrium_.
...
1. M. Maschler, E. Solan, and Shmuel Zamir, _Game Theory_, Cambridge University Press, 2013, p. 190.
