...
**BPP** (_bounded-error probabilistic polynomial time_) is the set of [[decision problem|languages]] that can be solved by [[probabilistic Turing machine]] in [[class P|polynomial time]] with _probability_ $\geq \frac{2}{3}$ of accepting an input in the language and _probability_ $\geq \frac{2}{3}$ of rejecting an input not in the language

## Definition (_almost-P_)
...
- _PRIMES is in P_ paper (AKS2004) shows that the problem of determining whether a given number is prime is in $\mathbf{P}$ (previously known to be in $\mathbf{BPP}$)
