...
An **Aumann model of incomplete information with beliefs** (over a set of *states of nature* $S$) consists of five elements $(N, Y, (\mathcal{F}_i)_{i \in N}, \mathfrak{s}, \mathbf{P})$
...
### Theorem
Let $(N, Y, (\mathcal{F}_i)_{i \in N}, \mathfrak{s}, \mathbf{P})$ be an *Aumann model of incomplete information with beliefs*. Then $\forall \omega \in Y$, $\forall i \in N$ (players), $\forall A \subseteq Y$ (events), player $i$ knows event $A$ in state of the world $\omega$ if and only if he attributes probability $1$ to that event:
