...
---
## Theorem
Let $A$ be any [[random variable]] with values in set of *deterministic algorithms* $\mathcal{A}$ and let $X$ be any *random variable* with values in *class of instances* $\mathcal{X}$. Suppose $x \in \mathcal{X}$ and $a \in \mathcal{A}$ induce *cost* $c(a,x) \in \mathbb{R}$. Let $A$ be a randomized algorithm (now a random variable), then, 
