...
---
## Definition
Suppose [[random variable|random variables]] $X$ and $Y$ with *support* $\mathcal{Z}$, then the **statistical distance** $\mathsf{SD}$ of $X,Y$ is $$\mathsf{SD}(X,Y) = \frac{1}{2}\sum_{z \in \mathcal{Z}} \lvert \Pr[X=z]-\Pr[Y=z]\rvert$$
