...
## Definition ($(\alpha,\beta)$_-spanner_)

Given $\alpha, \beta \geq 1$, and an arbitrary undirected unweighted _graph_ with $n$-nodes $G = (V,E)$, call a _spanning subgraph_ $G' \subseteq G$ an $(\alpha,\beta)$-**spanner** of $G$ if for every pair $s,t$ of vertices,
$$\operatorname{dist}_G(s,t) \leq \operatorname{dist}_{G'}(s,t) \leq \alpha \cdot \operatorname{dist}_G(s,t) + \beta$$

A **$t$-spanner** is another name for _$(t,0)$-spanner_ (used in some literature), where $t$ is defined as the _stretch factor_.
