...
- _envy-freeness up to one good_ (EF1), $\exists j \in x_k$ s.t. $v_i(x_i) \geq v_i(x_k\setminus \{j\})$
- _envy-freeness up to any good_ (EFX), $\forall j \in x_k$, $v_i(x_i) \geq v_i(x_k\setminus \{j\})$
...
- _envy-freeness with prioritized agents_ (EFprior) (Bu et al 2026)
  - suppose a set of prioritized agents $P \subseteq N$, then EFprior is when
    - allocation is EF1, and
    - $\forall i \in P$, $j \in N \setminus P$, $i$ does not _envy_ $j$
