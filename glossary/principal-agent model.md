...
- **principal-agent model**: a _principal_ interacts with an _agent_
- agent: set of _actions_ $\mathcal{A}$ of size $n$
  - action _costs_ for agent: $0 \leq c_1 \leq ... \leq c_n$
- agent's _technology_: probability matrix $\mathbf{q} = \{q_{ij}\}_{i \in [n],j \in [m]}$ describing stochastic outcome from agent's action
...
- _contract_: payment rule $\mathbf{t} = (t_1,...,t_m)$ of different _transfers_ for each of the $m$ outcomes
