...
A _relation_ $\leq$ is a **partial order** on a [[set]] $S$ if it satisfies:
...
A **partially ordered set** (_poset_) is an ordered pair $P = (X, \leq)$ consisting of a set $X$ (_ground set_ of $P$) and _partial order_ $\leq$ on $X$.
