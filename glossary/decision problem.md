...
## Definition (_mapping of strings to Boolean functions_)
...
## Definition (_set of words_)

A **language** $L \subseteq \Sigma^*$ is a set of [[word|words]] (where $\Sigma^*$ is the set of all words over [[alphabet]] $\Sigma$)

## Notes

- _Decision problem_ is the _formal language_ of all inputs (answer to yes-no question for given input) for which the output is YES.
...
1. S. Arora, B. Barak. _Computational Complexity: A Modern Approach_, Cambridge University Press, 2009, p. 3.
