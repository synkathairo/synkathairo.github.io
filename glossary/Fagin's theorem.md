...
A class $\mathcal{C}$ of _finite structures_ is definable by a sentence of _existential second-order logic_ if and only if it is decidable by a _nondeterministic machine_ running in _polynomial time_, i.e. [[universal second-order logic|USO]] = [[class coNP|co-NP]]
