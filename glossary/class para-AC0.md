...
## Definition (_para-AC⁰_)

Let $(Q,\kappa)$ be a [[parameterized problem]]. Then $(Q,\kappa)$ is in **para-$\mathbf{AC}^0$** if there exists a family $(\mathsf{C}_{n,k})_{n,k \in \mathbb{N}}$ of _boolean circuits_ ($n$ input gates and _parameter_ $k$) such that
...
4. there is a [[Turing machine|deterministic Turing machine]] that on input $(1^n, 1^k)$ computes the circuit $\mathsf{C}_{n,k}$ in time $g(k) + O(\log n)$, where $g: \mathbb{N} \to \mathbb{N}$ is a _computable function_

## See also

- [[class AC0|AC⁰]], for which this is the _parameterized_ version
