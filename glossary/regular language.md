...
The class $\operatorname{REG}(\Sigma)$ of **regular languages** is the _smallest class_ (i.e. [[set]] of) [[decision problem|languages]] (for [[alphabet]] $\Sigma$) such that

- $\emptyset \in \operatorname{REG}(\Sigma)$ and $\{a\} \in \operatorname{REG}(\Sigma)$ for every $a \in \Sigma$
- if $L,M \in \operatorname{REG}(\Sigma)$ then $L \cup M \in \operatorname{REG}(\Sigma)$ (_union_), $L \cdot M \in \operatorname{REG}(\Sigma)$ (_concatenation_), and $L^* \in \operatorname{REG}(\Sigma)$ ([[Kleene star]])
