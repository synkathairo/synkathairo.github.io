...
1. a specific _vertex_ indicating the starting point of the game
...
3. partition of the nodes of the tree into two _player sets_, $\overline{N}^1$ and $\overline{N}^2$ for P1 and P2 respectively
4. subpartition of each player set into _information sets_ $\{\eta_j^i\}$, such that the same number of intermediate branches emanates from every node belonging to the same information set, and no node follows another node in the same information set
...
- $(V, E, x^0)$ is a _tree_ called the _game tree_
- $(V_i)_{i \in N}$ is the _partition_ of the set of vertices that are not leaves
...
- the pair $(U_i^j, A(U_i^j))$ is an information set of player $i$ for each player $i \in N$ and $j \in \{1,2,...,k_i\}$
  Here, each element $U_i^j$ in the partition $(U_i^j)^{j = 1,...,k_i}$ of $V_i$ is an _information set_ of player $i$.
...
1. T. Başar and G.J. Olsder, _Dynamic Noncooperative Game Theory_, 2nd edition, Classics in Applied Mathematics, SIAM, Philadelphia, 1999, pp. 36-39.
2. M. Maschler, E. Solan, and Shmuel Zamir, _Game Theory_, Cambridge University Press, 2013, p. 43.
